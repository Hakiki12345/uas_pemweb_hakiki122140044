...
class SessionBasedAuthenticationPolicy:
    """Custom authentication policy for session-based authentication."""
    
    def authenticated_userid(self, request):
        """Return the user ID if the user is authenticated, None otherwise."""
        return request.session.get('user_id')
    
    def unauthenticated_userid(self, request):
        """Return the user ID without checking if they're authenticated."""
        return request.session.get('user_id')
    
    def effective_principals(self, request):
        """Return a list of principals for the user."""
        principals = [Everyone]
    ...
    def remember(self, request, userid, **kw):
        """Return headers for remembering the user (not used in session auth)."""
        return []
    
    def forget(self, request):
        """Return headers for forgetting the user (not used in session auth)."""
        return []
...
def main(global_config, **settings):
    ...
    config.set_session_factory(session_factory)    # Configure authentication and authorization policies
    authn_policy = SessionBasedAuthenticationPolicy()
